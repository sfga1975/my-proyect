...
if os.path.exists('solution.py'):
    from solution import DB_PATH, Task, ToDo, create_db
else:
    from main import DB_PATH, Task, ToDo, create_db  # type: ignore
...
def make_db_attrs_use_test_database(monkeypatch: pytest.MonkeyPatch):
    ...
    for class_ in CLASSES:
        autocommit = True if class_.con.autocommit is True else sqlite3.LEGACY_TRANSACTION_CONTROL
        row_factory = class_.con.row_factory if hasattr(class_.con, 'row_factory') else None
        monkeypatch.setattr(class_, 'con', sqlite3.connect(TEST_DB_PATH, autocommit=autocommit))  # type: ignore
        monkeypatch.setattr(class_.con, 'row_factory', row_factory)
...
def clean_user_database():
    try:
        ...
        if (p := Path(DB_PATH)).exists() and (p.stat().st_size == 0):
            p.unlink()
...
@pytest.mark.dependency(depends=['test_task_class_has_db_attrs'])
...
def test_task_representation(task1: Task, task2: Task, task3: Task):
    ...
    task2.save()
    assert repr(task2) == '[X] Estudiar bases de datos (id=2)'

    task3.save()
    assert repr(task3) == '[X] Estudiar sistemas informáticos (id=3)'
...
@pytest.mark.dependency(depends=['test_task_class_has_db_attrs'])
def test_get_task_returns_None_if_task_does_not_exist():
    task = Task.get(999)
    assert task is None
...
@pytest.mark.dependency(depends=['test_todo_class_has_db_attrs'])
...
@pytest.mark.dependency(depends=['test_task_class_has_db_attrs', 'test_todo_class_has_db_attrs'])
